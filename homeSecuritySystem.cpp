...
const float FEELS_LIKE_THRESHOLD = 35.0;
...
void handleTemperatureHumidity() {
  ...
  if (temp > TEMP_THRESHOLD || hum > HUM_THRESHOLD || feelsLikeTemp > FEELS_LIKE_THRESHOLD) {
    triggerAlarm();
...
bool isKnownRFIDCard() {
  byte knownUID[] = {0xDE, 0xAD, 0xBE, 0xEF}; 
...
float calculateFeelsLikeTemp(float temp, float hum) {
  return -8.784695 + 1.61139411 * temp + 2.338549 * hum - 0.14611605 * temp * hum;
}
